...
export const MANUAL_SECTIONS: ManualSection[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      'Creadores → "Buscar en vivo": "Por @usuario" analiza ese perfil con Apify (~10 s). "Por categoria" busca primero GRATIS en nuestra base (categoría asignada, palabras de la bio o nombre; solo creadores con datos reales) y muestra el recuento "De nuestra base de datos"; debajo, "Buscar más en Apify" abre una confirmación sin cifras de coste → "Sí, buscar en Apify" o "Cancelar". Al confirmar lanza una única búsqueda de publicaciones recientes del hashtag de la categoría, completa los autores nuevos con sus datos de perfil en esa misma ejecución y guarda el resultado 7 días (repetirla no consume). Si Apify está pausado por el límite mensual, el botón no aparece y sale un aviso sin cifras; la base de datos sigue funcionando. "Pegar lista de handles" → "Procesar todos" analiza muchos perfiles de golpe.',
    ...
    troubleshooting: [
      ...
      },
      {
        ...
        doWhat: [
          ...
          'Cuando Apify se acerca a su límite mensual las stories se paran; con el plan agotado se para todo hasta el siguiente ciclo.',
...
const MANUAL_DIGEST: Record<string, string> = {
  acceso:
    'Login en intelligence.thekingofcontent.agency; invitaciones en Ajustes → Equipo. Roles: ADMIN (todo), EMPLOYEE (PM), BRAND (cliente, solo lectura en /portal), CREATOR. "TKOC AI" explica cómo hacer las cosas; no ejecuta acciones. Acceso del cliente: Marcas → "Usuario Marca (solo lectura)".',
  'crear-campana':
  ...
  informe:
    'Ficha → "Informe (PDF)". "Editar informe": Título, Subtítulo, Introducción, Conclusiones; "Secciones — Ocultar al cliente"; "Columnas y tarjetas — Ocultar al cliente"; en Creadores/Anexo "Ocultar esta fila al cliente" ("Oculto al cliente", totales recalculados). "Prometido vs entregado": "Creadores que han publicado", "Piezas publicadas", "Publicadas dentro del periodo de campaña", "Identificación legal"; Modo "Automático" / "Manual" (Prometido, Entregado, Estado "Cumplido" / "En revisión", Nota corta); "Compromisos adicionales" → "Añadir compromiso" (máx. 4). Al cliente solo llegan las filas "Cumplido". Identificación legal: feed (no stories) con #publicidad / #publi / "Publi" / colaboración pagada / #ad. "Comentarios destacados" → "Añadir comentario" (máx. 12). "Marcar como enviado" → "Confirmar envío" → "Versión N enviada el…". "Descargar PDF" = versión cliente idéntica a la pantalla sin Balance, "Datos: qué es real", coste, CPM ni Ratio EMV; "Imprimir" es secundario.',
  portal:
  ...
  'contactos-listas':
    'Contactos se llena solo al añadir creadores ("Editar dirección"). Listas → "Crear nueva Lista"; dentro solo "Eliminar de la lista" y fijar (sin "Añadir a Campaña"; "Exportar" no hace nada). Creadores → "Base de datos": solo creadores con datos reales (seguidores > 0), sin Apify; filtros texto, plataforma, categoría, ciudad, seguidores, encaje con España. "Buscar en vivo": "Por @usuario" (Apify, ~10 s) o "Por categoria" (primero gratis en nuestra base, "De nuestra base de datos"; "Buscar más en Apify" → confirmación sin cifras → "Sí, buscar en Apify" / "Cancelar" → una ejecución, guardada 7 días; con Apify pausado no hay botón). Por resultado "Ver perfil" y "Añadir a..." (solo a una lista; a campaña desde Elegir o la ficha). Lo analizado o capturado entra en la base con su categoría. Similares y "Buscar Similares" (Analizar Perfil): mismo motor, solo nuestra base; primero afinidad temática, después "Audiencia comparable (sin afinidad temática confirmada)"; fuente sin categoría → todos "Audiencia comparable (el creador fuente no tiene categoría asignada)" (analízalo antes); "Razones de coincidencia"; Apify solo para "Añadir a lista" sin ficha (analiza primero) y para el fuente fuera de la base. Nunca cites cifras de coste de Apify.',
  ajustes:
    'Pestañas Perfil, Equipo, Integraciones, Benchmarks (solo ADMIN), Facturación. Equipo → "Invitar Miembro" → "Enviar Invitación". Integraciones: Apify ("Probar Conexión"), Meta ("Connect with Facebook", "Reconnect", "Sincronizar"). Benchmarks → "Tasas EMV": "Tasas CPM (por 1.000 de audiencia)" por plataforma y formato, "Stories: audiencia estimada (% de seguidores)", "Publicaciones: alcance estimado (% de seguidores)", "Valor por vista (reels y vídeos)" (0 = CPM; hoy 0,05 € en reels), "Multiplicador de confianza" (×1 = apagado, hoy apagado). Fees, CPM aceptable, mercados y modificadores: cerrados, solo David.',
  'que-hacer-si':
    'Comprueba primero las reglas de captura (miembro, fechas, mención), la campaña activa y el estado de Apify/Meta.',
}
...
const MANUAL_FIX_DIGEST: Record<string, string> = {
  ...
  'No salen stories':
    'creadora en Acordado+, campaña activa, dentro de fechas y ≤ 62 días; escaneo cada 12 h (story caduca a 24 h: si se pierde, "Registrar Story"); con Apify cerca de su límite mensual o agotado las stories se paran.',
  'Las miniaturas salen en blanco':
    'las URLs caducan; hay copia duradera al capturar y en Instagram (no TikTok) se recuperan solas sin Apify; recarga en unos minutos y, si persiste, avisa a David con la URL.',
  'Apify está agotado':
    'banner en Creadores; sin "Por @usuario", "Buscar más en Apify" ni scraping hasta el nuevo ciclo (o ampliar en Apify → Billing, David); siguen "Base de datos", "Por categoria" en la base, Similares, Meta y lo guardado. Prioriza "Invitar a conectar" y capturas.',
  ...
  'Una dirección no se rellena en el modal de envío':
    'solo se rellena si Contactos guarda una dirección y esta campaña aún no tiene; si dice "Sin dirección", tecléala y "Guardar Datos"; si la hay, reabre "Datos de Envío" o usa "Editar dirección".',

...
const MANUAL_DIGEST: Record<string, string> = {
  acceso:
    'Login en intelligence.thekingofcontent.agency; invitaciones en Ajustes → Equipo. Roles: ADMIN (todo), EMPLOYEE (PM), BRAND (cliente, solo lectura en /portal), CREATOR. "TKOC AI" explica cómo hacer las cosas; no ejecuta acciones. Acceso del cliente: Marcas → "Usuario Marca (solo lectura)". Campañas: cada PM ve las que creó o con acceso; ADMIN ve todas. Dar acceso: ficha → "Equipo" → "Tiene acceso" (solo creador o ADMIN); la PM recibe "Acceso a campaña". Campana: solo avisos de tus campañas (creadas o con acceso), nunca a todo el equipo; alertas de sistema solo ADMIN.',
  'crear-campana':
...
const MANUAL_FIX_DIGEST: Record<string, string> = {
  ...
  'El CPM no aparece':
    'sin "Fee Acordado (€)" no hay coste y sin vistas reales no hay base. Es interno: el cliente no lo ve.',
  ...
  'No salen stories':
    'creadora en Acordado+, campaña activa, en fechas y ≤ 62 días; escaneo cada 12 h (caduca a 24 h: si se pierde, "Registrar Story"); con Apify cerca del límite o agotado se paran.',
  'Las miniaturas salen en blanco':
    'las URLs caducan; hay copia duradera y en Instagram (no TikTok) se recuperan solas; recarga y, si persiste, avisa a David con la URL.',
  'Apify está agotado':
    'banner en Creadores; sin "Por @usuario", "Buscar más en Apify" ni scraping hasta el nuevo ciclo (o David amplía en Apify → Billing); siguen "Base de datos", "Por categoria", Similares y Meta. Prioriza "Invitar a conectar" y capturas.',
  ...
  'No veo una campaña que lleva otra PM':
    'solo la ven su creador y quien tiene acceso (ADMIN, todas); pide al creador o a un ADMIN: ficha → "Equipo" → "Tiene acceso"; te llega "Acceso a campaña". Marcas → "Gestionar Empleados" no da acceso.',

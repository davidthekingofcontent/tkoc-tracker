...
export const MANUAL_SECTIONS: ManualSection[] = [
  {
    ...
    steps: [
      ...
      'Roles: ADMIN (todo, incluidas Integraciones y Benchmarks), EMPLOYEE (tú como PM: campañas, creadores, listas, contactos, informes), BRAND (el cliente: solo lectura, confinado en /portal) y CREATOR (panel propio).',
  ...
  },
  {
    ...
    steps: [
      ...
      'También desde Creadores (modos "Base de datos" y "Buscar en vivo") → "Añadir a..." y desde Similares o Analizar Perfil → "Añadir a lista".',
  ...
  },
  {
    ...
    steps: [
      ...
      'Stories de Instagram: caducan a las 24 h. Se escanean cada 12 h, en UNA sola ejecución para todas las creadoras en Acordado o superior de las campañas activas dentro de fechas y de 62 días o menos (las largas no se escanean: se paga por cuenta). Las menciones con sticker llegan al instante vía Meta si la marca tiene conectado su Instagram.',
      'Ejecutar → Media → "Añadir publicación por URL": pega la URL, "Detectar desde la URL" y "Añadir publicación". El creador debe ser miembro y la fecha estar dentro de la campaña; si no, se rechaza con un mensaje.',
      'Ejecutar → Stories → "Registrar Story" para anotar a mano una story: Influencer, Vistas, Alcance, Respuestas, Enlace (opcional) → "Guardar Story". El dato manual manda y nunca lo borra la revalidación.',
  ...
  },
  {
    ...
    steps: [
      ...
      'Listas → "Crear nueva Lista". Dentro de una lista: "Eliminar de la lista" y fijar la lista (aparece en la barra lateral). No hay "Añadir a Campaña" y el botón "Exportar" todavía no hace nada: para llevar sus creadores a una campaña, usa Creadores → "Añadir a..." o la ficha del creador → "Anadir a campana".',
      'Creadores: "Base de datos" (búsqueda interna instantánea por nombre, categoría, ciudad, país, seguidores, engagement, encaje con España) o "Buscar en vivo" (Apify, 1-2 minutos). "Pegar lista de handles" → "Procesar todos" analiza muchos perfiles de golpe. Por resultado: "Ver perfil", "Añadir a lista", "Enriquecer perfiles".',
      'Analizar Perfil (/analyze): pega la URL o @handle → "Analizar" → estadísticas, "Obtener Insights", "Buscar Similares", "Añadir a lista", "Tarjeta de Tarifas" (tarifa estándar).',
      'Similares: introduce un @handle o URL → candidatos parecidos con "Razones de coincidencia" → "Ver perfil" / "Añadir a lista". Fuera de la base de datos necesita Apify.',
  ...
  },
  {
    ...
    steps: [
      ...
      'Ajustes → Equipo → "Invitar Miembro" → email y Rol (Admin, Employee o Brand) → "Enviar Invitación". Aquí también se cambia el rol, se elimina y se asignan marcas a empleados.',
  ...
  },
  {
    ...
    steps: [
      ...
      'Después, el estado de Apify (banner en Creadores) y de las cuentas de Meta (Ajustes → Integraciones).',
    ...
    troubleshooting: [
      {
        ...
        doWhat: [
          ...
          'Creadores: ¿hay banner de Apify agotado? Entonces espera al ciclo o usa Meta.',
      ...
      },
      {
        ...
        doWhat: [
          ...
          'Por encima de 40 $ de gasto mensual de Apify las stories se paran; con el plan agotado se para todo hasta el siguiente ciclo.',
      ...
      },
      {
        ...
        doWhat: [
          ...
          'Mientras dura: no funciona "Buscar en vivo", analizar perfiles nuevos ni la captura por scraping; sí funciona todo lo que llega por Meta y lo ya guardado.',
...
const MANUAL_DIGEST: Record<string, string> = {
  acceso:
    'Login en intelligence.thekingofcontent.agency; invitaciones en Ajustes → Equipo → "Invitar Miembro". Roles: ADMIN (todo), EMPLOYEE (PM), BRAND (cliente, solo lectura en /portal), CREATOR. "TKOC AI" explica cómo hacer las cosas; no ejecuta acciones. Acceso del cliente: Marcas → "Usuario Marca (solo lectura)".',
  'crear-campana':
    'Campañas → "Nueva Campaña" ("Cargar desde plantilla" opcional). "Marca / Cliente" (sin marca el cliente no ve la campaña). Tipo: "Seguimiento de Influencers", "Social Listening" o "Campaña UGC". "Objetivo de la campaña" (obligatorio salvo Social Listening) y "Objetivos numéricos" (al menos uno > 0: Vistas, Alcance, Interacciones, ER objetivo, CPM máximo); si falta: "Selecciona un objetivo y al menos un objetivo numérico para crear la campaña." "Cuentas de Marca Objetivo" (@menciones y #hashtags; sin ellos no se captura nada). "Campaña de Pago" / "Campaña Gifted". "Fecha de Inicio" / "Fecha de Fin" o "Siempre Activo": solo se captura lo publicado dentro. Brief. "Crear Campaña". Los objetivos se congelan al arrancar; cambiarlos en Aprender → "Editar Campaña" pide "Motivo del cambio" ("Historial de cambios").',
  'creadores-trato':
    'Ficha → Elegir: "Buscar por username...", "Añadir Creador UGC", "Añadir varios" ("Pegar lista" u "O sube un CSV", hasta 200). Estados (selector en la fila; Ejecutar → Pipeline; Pipeline global): Prospecto → Contacto → Negociando → Acordado → Contratado → Envío → Publicado → Completado. Poner Acordado al confirmar: activa el rastreo de stories, congela la línea base y cuenta en "Prometido vs entregado"; en Prospecto no se rastrean stories. "Fee solicitado (€)", "Fee Acordado (€)" (= coste en todas las métricas), "Formato" negociado. "Entregables comprometidos", "Enlace con UTM", "Clics" ("Entregados N / M"). "Invitar a conectar", "Quitar de campaña". Pagar → "Análisis de Pricing por Influencer". Línea base automática al pasar a Acordado (mín. 6 publicaciones); si "Sin línea base": "Introducir línea base".',
  envios:
    'Estado Envío → Ejecutar → Envíos → "Añadir envío" / "Editar envío" → modal "Datos de Envío" (Nombre destinatario, Dirección, Ciudad, Código Postal, País, Teléfono, Email, Producto / SKU, Cantidad) → "Guardar Datos". Si Contactos ya tiene dirección se rellena solo ("Dirección guardada en Contactos el {fecha}"); al guardar se copia a Contactos. "Descargar CSV Envíos". Corregir: Contactos → "Editar dirección".',
  capturar:
    'Automático: posts cada 6 h (Apify), Meta cada 2 h (reels etiquetados entran con 0 vistas y se completan con el dato público), borrados cada 24 h, captura inmediata al añadir o pasar a Acordado+. Reglas: miembro + dentro de fechas + menciona Cuenta de Marca Objetivo o hashtag. "Rastrear Ahora" (campaña activa; siempre ejecuta, sin ventana de 3 h). Stories: cada 12 h, una sola ejecución, creadoras en Acordado+ de campañas activas dentro de fechas y ≤ 62 días; caducan a las 24 h. Ejecutar → Media → "Añadir publicación por URL" → "Detectar desde la URL" → "Añadir publicación". Ejecutar → Stories → "Registrar Story" (Vistas, Alcance, Respuestas) → "Guardar Story". "Registrar estadísticas" por publicación: captura de la creadora → "Leer con IA" → revisar → guardar ("Estadísticas del creador"), o a mano. "Etiquetas" (Enfoque, Gancho, Beneficio de producto). "Revalidar contenido" desvincula lo que no cumple.',
  'datos-reales':
    'Interacciones = likes + comentarios + compartidos + guardados. Vistas ≥ likes o "sin dato real". ER = interacciones ÷ vistas reales × 100; se publica con ≥ 3 publicaciones con vistas, ≥ 500 vistas y ratio ≤ 100 %; si no, "Muestra real insuficiente"; nunca sobre seguidores, alcance ni estimaciones. Audiencia real = alcance → impresiones → vistas reales; la estimada es informativa, solo agencia. Coste = fee acordado; CPM real = coste ÷ vistas × 1.000 (interno). EMV: una sola cifra "EMV" con "?" (valor equivalente en medios pagados; reels/vídeos con vistas reales × valor por vista o CPM, más interacciones; stories sin vistas por tier); nunca "estimado". Solo agencia: fees, coste, CPM, EMV Básico, Ratio EMV (×, nunca ROI), estimada, impresiones, Balance, CPA/ROAS. Alcance, guardados, compartidos y vistas de stories solo llegan del creador ("Registrar estadísticas", "Invitar a conectar" o manual).',
  'objetivos-negocio':
    'Planificar → bloque "Resumen" → "Objetivos de campaña": KPI · Objetivo · Resultado · Variación; ±10 %: "En objetivo", "Por encima del objetivo", "Por debajo del objetivo", "Sin datos"; el cliente no ve la fila de CPM máximo. Cambiar: Aprender → "Editar Campaña" → "Motivo del cambio". Planificar → "Resultados de negocio (aportados por el cliente)": Código promocional, Canjes, Ventas, Leads, Ingresos, Fuente, Fecha, Notas, "ROI (dato del cliente)"; vacío = no aparece. CPA/ROAS solo agencia. Clics: Elegir → "Enlace con UTM" → "Clics".',
  informe:
    'Ficha → "Informe (PDF)". "Editar informe": Título, Subtítulo, Introducción, Conclusiones; "Secciones — Ocultar al cliente"; "Columnas y tarjetas — Ocultar al cliente" ("Creadores · Coste y CPM (interno; nunca en el PDF ni en el portal)"); en Creadores/Anexo "Ocultar esta fila al cliente" ("Oculto al cliente", totales recalculados). "Prometido vs entregado": "Creadores que han publicado", "Piezas publicadas", "Publicadas dentro del periodo de campaña", "Identificación legal"; Modo "Automático" / "Manual" (Prometido, Entregado, Estado "Cumplido" / "En revisión", Nota corta); "Compromisos adicionales" → "Añadir compromiso" (máx. 4). Al cliente solo llegan las filas "Cumplido". Identificación legal: feed (no stories) con #publicidad, #publi, "Publi", colaboración pagada, #ad o etiqueta de colaboración pagada. "Comentarios destacados" → "Añadir comentario" (máx. 12). "Marcar como enviado" → "Confirmar envío" → "Versión N enviada el…". "Descargar PDF" = versión cliente idéntica a la pantalla sin Balance, "Datos: qué es real", coste, CPM ni Ratio EMV; "Imprimir" es secundario.',
  portal:
    'Acceso: campaña con "Marca / Cliente" + Marcas → "Usuario Marca (solo lectura)". Ve: campañas con estado, contenido (borrados marcados), creadores con estado en su lenguaje (Propuesto… Confirmado… Publicado), vistas, interacciones, audiencia real, ER sobre vistas, entregables comprometidos, "EMV" con "?", objetivos sin CPM, sus resultados de negocio, aprendizajes versión cliente, "Ver informe completo" con "Descargar PDF". No ve: fees, coste, CPM, Ratio EMV, EMV Básico, CPA, ROAS, estimada, impresiones, Balance, nota de campaña, peor creador, presupuesto. No edita ni rastrea.',
  aprender:
    'Aprender → "Inteligencia de Campaña"; "Playbook — Qué hacer a continuación" ("Repetir" / "No repetir la próxima vez", "Mejor rendimiento", "Rendimiento por formato", "Consejo de presupuesto", "Siguiente campaña"); "Sin vistas reales" = creadores sin vistas reales (≥ likes, mín. 500): no se juzgan, pide estadísticas. Informe → "Aprendizajes y próximos pasos": "Qué repetir" (creadores a repetir; "Todavía no destaca ningún creador" sin dato real), "Mejor rendimiento", "Formato ganador", "Siguiente oleada" (recomendación para la próxima campaña); bloque "Solo agencia" no se imprime. Mismos datos que el informe.',
  'contactos-listas':
    'Contactos se llena solo al añadir creadores ("Editar dirección"). Listas → "Crear nueva Lista"; en la lista solo "Eliminar de la lista" y fijar (sin "Añadir a Campaña"; "Exportar" no hace nada). Creadores: "Base de datos" o "Buscar en vivo" (Apify); "Pegar lista de handles" → "Procesar todos"; "Ver perfil", "Añadir a lista", "Enriquecer perfiles". Analizar Perfil y Similares para encontrar más.',
  ajustes:
    'Pestañas Perfil, Equipo, Integraciones, Benchmarks (solo ADMIN), Facturación. Equipo → "Invitar Miembro" → "Enviar Invitación". Integraciones: Apify ("Probar Conexión"), Meta ("Connect with Facebook", "Reconnect", "Sincronizar"). Benchmarks → "Tasas EMV": "Tasas CPM (por 1.000 de audiencia)" (Instagram post 10 €, reel 14 €, story 8 €; TikTok vídeo 8 €; YouTube vídeo 15 €, short 8 €), "Stories: audiencia estimada (% de seguidores)", "Publicaciones: alcance estimado (% de seguidores)", "Valor por vista (reels y vídeos)" (0 = CPM; hoy 0,05 € en reels), "Multiplicador de confianza" (×1 = apagado, hoy apagado); "Restaurar valores por defecto". Fees, CPM aceptable, mercados y modificadores: cerrados, solo David.',
  'que-hacer-si':
    'Comprueba primero las reglas de captura (miembro, fechas, mención), que la campaña esté activa y el estado de Apify/Meta. Si falta alcance, guardados o vistas de stories: "Registrar estadísticas".',
}
...
const MANUAL_FIX_DIGEST: Record<string, string> = {
  'Una creadora no muestra publicaciones':
    'cuentas objetivo en la campaña → es miembro en Elegir → publicación dentro de fechas y con mención/hashtag (si no, "Añadir publicación por URL") → Apify no agotado → "Rastrear Ahora" o "Sincronizar" en Integraciones.',
  'La tasa de engagement dice "Muestra real insuficiente"':
    'faltan ≥ 3 publicaciones con vistas reales, 500 vistas y ratio plausible; pide capturas y usa "Registrar estadísticas", "Invitar a conectar" o "Registrar Story". Nunca se rellena con estimaciones.',
  'El CPM no aparece':
    'sin "Fee Acordado (€)" no hay coste y sin vistas reales no hay base; rellena fees y consigue vistas reales. Es interno: el cliente no lo ve.',
  'La checklist "Prometido vs entregado" sale ámbar':
    'estados Acordado+, "Entregables comprometidos" rellenos, fechas dentro del periodo, #publicidad en el texto; si el sistema se equivoca, en "Editar informe" pasa la fila a Manual con Estado "Cumplido" y Nota corta.',
  'Una fila no llega al cliente':
    'solo salen las filas "Cumplido" (corrige o pásala a Manual → "Cumplido"); en Creadores/Anexo quita "Oculto al cliente" con "Volver a mostrar al cliente"; coste y CPM nunca salen.',
  'El PDF muestra algo que no quiero':
    '"Editar informe" → ocultar sección, columna o fila → guardar → "Descargar PDF" (siempre versión cliente; usa "Descargar PDF", no "Imprimir").',
  'No salen stories':
    'creadora en Acordado+, campaña activa, dentro de fechas y ≤ 62 días; escaneo cada 12 h (story caduca a 24 h: si se pierde, "Registrar Story"); con Apify por encima de 40 $/mes o agotado las stories se paran.',
  'Las miniaturas salen en blanco':
    'las URLs de Instagram/TikTok caducan; la plataforma guarda copia duradera al capturar y en Instagram (no TikTok) recupera sola las caducadas sin Apify; recarga en unos minutos y, si persiste, avisa a David con la URL.',
  'Apify está agotado':
    'banner en Creadores; no hay "Buscar en vivo" ni scraping hasta el nuevo ciclo (o ampliar en Apify → Billing, David); sigue funcionando Meta y lo guardado. Prioriza "Invitar a conectar" y capturas.',
  ...
  'El cliente no ve la campaña en el portal':
    'campaña con "Marca / Cliente" (solo al crearla, no en "Editar Campaña"); Marcas → "Usuario Marca (solo lectura)" vinculado a esa marca con rol Brand; entra por /portal.',
  'Una dirección no se rellena en el modal de envío':
    'solo se rellena si no hay dirección en esta campaña y Contactos guarda una; si Contactos dice "Sin dirección", tecléala y "Guardar Datos"; si la hay, reabre "Datos de Envío" o corrígela con "Editar dirección".',
  ...
  'La ficha y el informe (o el portal) muestran cifras distintas':
    'solo es legítimo si ocultaste filas en el informe o el cliente no ve cifras económicas; si no, recarga y avisa a David.',
}
...
function renderManualText(sections: ManualSection[]): string {
  ...
  return `# Manual de la PM (rótulos reales de la interfaz; el manual completo está en /manual)\n${blocks.join('\n')}`
}
